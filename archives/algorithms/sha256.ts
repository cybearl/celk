...
export class Sha256Algorithm_v1 {
     ...
     */
    private sha256 = (cache: Cache, input: Cache, offset: number): void => {
        const HASH = [
        ...
        cache.writeUint32BEArray(HASH, offset);
    };
     ...
     */
    execute = (cache: Cache, slot: MemorySlot): void => {
        // Empty the input array by keeping the reference
        this.inputArray.length = 0;

        const input = cache.subarray(slot.readFrom.offset, slot.readFrom.end);
        this.cacheToBigEndianWords(input);
        this.sha256(cache, input, slot.writeTo.offset);
    };
}