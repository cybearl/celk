// Constants
export const POW_2_128 = 2n ** BigInt(128);

/**
 * SECG SECP256K1 elliptic curve parameters.
...
/**
 * Split scalar endomorphism constants
 */
export const SPLIT_1 = BigInt("0x3086d221a7d46bcde86c90e49284eb15");
export const SPLIT_2 = BigInt("0xe4437ed6010e88286f547fa90abfe4c3");
export const SPLIT_3 = BigInt("0x114ca50f7a8e2f3f657c1108d9d44cfd8");
 ...
 * @returns The sum.
 */
export function mod(a: bigint, b = CURVE.P): bigint {
    const result = a % b;
 ...
 * @returns The inverse.
 */
export function invert(number: bigint, modulo = CURVE.P): bigint {
    if (number === 0n || modulo <= 0n) {
        throw new Error(`[ecdsa] invert: Expected positive integers, got n=${number} mod=${modulo}`);
    }
    ...
    const gcd = b;
    if (gcd !== 1n) throw new Error("[ecdsa] invert: Does not exist");
 ...
 */
export function isWithinCurveOrder(num: bigint): boolean {
 ...
 */
export function normalizeScalar(num: number | bigint): bigint {
    if (typeof num === "number" && num > 0 && Number.isSafeInteger(num)) return BigInt(num);
    if (typeof num === "bigint" && isWithinCurveOrder(num)) return num;

    throw new TypeError("[ecdsa] normalizeScalar: Expected valid private scalar: 0 < scalar < curve.n");
}
 ...
 */
export function divNearest(a: bigint, b: bigint): bigint {
    return (a + b / 2n) / b;
}
 ...
 */
export function splitScalarEndomorphism(k: bigint): {
    k1neg: boolean;
    k1: bigint;
    k2neg: boolean;
    k2: bigint;
    ...
    if (k1 > POW_2_128 || k2 > POW_2_128) {
        throw new Error("[ecdsa] splitScalarEndomorphism: Endomorphism failed");
    }
...
export class JacobianPoint {
    ...
    add = (other: JacobianPoint): JacobianPoint => {
        if (!(other instanceof JacobianPoint)) {
            throw new Error("[JacobianPoint] add: expected JacobianPoint");
        }
    ...
    multiplyUnsafe = (scalar: bigint): JacobianPoint => {
        ...
        const split = splitScalarEndomorphism(n);
