...
/**
 * @brief The base struct for all messages sent between the main process and the worker.
 */
...
/**
 * @brief The way that addresses are represented inside the protocol (inline payload).
 */
struct AddressDump {
    std::string id;
    std::string name;
...
/**
 * @brief The type for closest matches attached to addresses inside a report message,
