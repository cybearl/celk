        ...
        await User.createMany([
            {
                ...
                isLocked: false,
                email: env.get("BASE_ADMIN_EMAIL"),
                username: env.get("BASE_ADMIN_USERNAME"),
                password: env.get("BASE_ADMIN_PASSWORD"),
                description: "The default administrator.",
            },
        ])

        // Link the default administrator to the 'admin' role using the pivot table
        const admin = await User.findBy("email", process.env.BASE_ADMIN_EMAIL)
