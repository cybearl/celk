...
export default class Cache extends Uint8Array {
    ...
    check = (offset: number, length: number): void => {
        if (Number.isNaN(offset) || Number.isNaN(length)) throw new TypeError(`[Cache - check] Invalid offset: '${offset}' or length: '${length}'.`);
        if (offset < 0 || offset >= this.length) throw new RangeError(`[Cache - check] Invalid offset: '${offset}', it must be >= 0 & < ${this.length}.`);
        if (length < 1 || length > this.length) throw new RangeError(`[Cache - check] Invalid length: '${length}', it must be > 0 & <= ${this.length}.`);
        if (offset + length > this.length) throw new RangeError(`[Cache - check] Invalid offset (${offset}) + length (${length}): '${offset + length}', it must be <= ${this.length}.`);
    ...
    *[Symbol.iterator]() {
    ...
    writeHexString = (value: string, offset = 0, length = value.length): this => {
        if (value.length === 0) throw new RangeError(`[Cache - writeHexString] Invalid hexadecimal string length: '${value.length}'.`);
        if (value.length % 2 !== 0) throw new RangeError(`[Cache - writeHexString] Invalid hexadecimal string length: '${value.length}'.`);
        ...
        for (let i = 0; i < length; i += 2) {
            ...
            this[offset + i / 2] = (highNibble - (highNibble > 57 ? 87 : 48)) << 4 | (lowNibble - (lowNibble > 57 ? 87 : 48));
        }
    ...
    writeUtf8String = (value: string, offset = 0, length = value.length): this => {
        if (value.length === 0) throw new RangeError(`[Cache - writeUtf8String] Invalid UTF-8 string length: '${value.length}'.`);
    ...
    writeUint16LE = (value: number, byteOffset = 0): this => {
        if (value < 0 || value > 0xFFFF) throw new RangeError(`[Cache - writeUint16LE] Value is out of bounds: '${value}'.`);
        if (byteOffset % 2 !== 0) throw new RangeError(`[Cache - writeUint16LE] Invalid byte offset alignment: '${byteOffset}' (%2).`);
    ...
    writeUint16BE = (value: number, byteOffset = 0): this => {
        if (value < 0 || value > 0xFFFF) throw new RangeError(`[Cache - writeUint16BE] Value is out of bounds: '${value}'.`);
        if (byteOffset % 2 !== 0) throw new RangeError(`[Cache - writeUint16BE] Invalid byte offset alignment: '${byteOffset}' (%2).`);
        this.check(byteOffset, 2);
    ...
    writeUint32LE = (value: number, byteOffset = 0): this => {
        if (value < 0 || value > 0xFFFFFFFF) throw new RangeError(`[Cache - writeUint32LE] Value is out of bounds: '${value}'.`);
        if (byteOffset % 4 !== 0) throw new RangeError(`[Cache - writeUint32LE] Invalid byte offset alignment: '${byteOffset}' (%4).`);
    ...
    writeUint32BE = (value: number, byteOffset = 0): this => {
        if (value < 0 || value > 0xFFFFFFFF) throw new RangeError(`[Cache - writeUint32BE] Value is out of bounds: '${value}'.`);
        if (byteOffset % 4 !== 0) throw new RangeError(`[Cache - writeUint32BE] Invalid byte offset alignment: '${byteOffset}' (%4).`);
        this.check(byteOffset, 4);
    ...
    writeUint8Array = (array: Uint8Array, offset = 0, length = array.length): this => {
        if (!array || !(array instanceof Uint8Array)) throw new TypeError(`[Cache - writeUint8Array] Invalid Uint8Array: '${array}'.`);
        this.check(offset, length);
     ...
     * @returns The cache instance.
     */
    writeUint16Array = (array: Uint16Array, byteOffset = 0, byteLength = array.byteLength, endianness = this.platformEndianness): this => {
        if (!array || !(array instanceof Uint16Array)) throw new TypeError(`[Cache - writeUint16Array] Invalid Uint16Array: '${array}'.`);
        if (byteOffset % 2 !== 0) throw new RangeError(`[Cache - writeUint16Array] Invalid byte offset alignment: '${byteOffset}' (%2).`);
     ...
     * @returns The cache instance.
     */
    writeUint32Array = (array: Uint32Array, byteOffset = 0, byteLength = array.byteLength, endianness = this.platformEndianness): this => {
        if (!array || !(array instanceof Uint32Array)) throw new TypeError(`[Cache - writeUint32Array] Invalid Uint32Array: '${array}'.`);
        if (byteOffset % 4 !== 0) throw new RangeError(`[Cache - writeUint32Array] Invalid byte offset alignment: '${byteOffset}' (%4).`);
        this.check(byteOffset, byteLength);
    ...
    readUint16LE = (byteOffset = 0): number => {
        if (byteOffset % 2 !== 0) throw new RangeError(`[Cache - readUint16LE] Invalid byte offset alignment: '${byteOffset}' (%2).`);
        this.check(byteOffset, 2);
    ...
    readUint16BE = (byteOffset = 0): number => {
        if (byteOffset % 2 !== 0) throw new RangeError(`[Cache - readUint16BE] Invalid byte offset alignment: '${byteOffset}' (%2).`);
        this.check(byteOffset, 2);
    ...
    readUint32LE = (byteOffset = 0): number => {
        if (byteOffset % 4 !== 0) throw new RangeError(`[Cache - readUint32LE] Invalid byte offset alignment: '${byteOffset}' (%4).`);
        this.check(byteOffset, 4);

        return (this[byteOffset] |
            this[byteOffset + 1] << 8 |
            this[byteOffset + 2] << 16 |
            this[byteOffset + 3] << 24) >>> 0;
    };
    ...
    readUint32BE = (byteOffset = 0): number => {
        if (byteOffset % 4 !== 0) throw new RangeError(`[Cache - readUint32BE] Invalid byte offset alignment: '${byteOffset}' (%4).`);
        this.check(byteOffset, 4);

        return (this[byteOffset] << 24 |
     ...
     */
    subarray = (offset = 0, length = this.length): Cache => new Cache(length, {
        buffer: this.buffer,
        offset: offset,
        length: length
    });
