...
export default function executeSha256AlgorithmBenchmark(benchmarkInputSize: number, benchmarkDuration: number) {
    ...
    bench.benchmark(() => Sha256Algorithm.hash(cache, inputSlot, outputSlot), `hash(${benchmarkInputSize})`)
    bench.benchmark(() => Sha256Algorithm.hash(cacheX1, X1InputSlot, X1OutputSlot), `hash(1)`)
    bench.print()
}
